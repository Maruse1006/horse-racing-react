...
export default function TrioThirdKeyScreen() {
  ...
  const calculateCombinations = () => {
    ...
    for (let fixedThird of firstRow) { // 2着を固定 (firstRow)
      for (let i = 0; i < secondRow.length; i++) { // secondRowで1着を選ぶ
        for (let j = 0; j < secondRow.length; j++) { // secondRowで3着を選ぶ
          if (i !== j) { // 1着と2着が同じ馬でないようにする
            const first = secondRow[i]; // 1着
            const second = secondRow[j]; // 3着
  
            // 2着(fixedSecond)が1着または2着と同じでないことを確認
            if (fixedThird !== first && fixedThird !== second) {
              combinations.push([first,second,fixedThird]); // 組み合わせを作成
